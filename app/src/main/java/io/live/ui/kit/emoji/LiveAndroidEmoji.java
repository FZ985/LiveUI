...
public class LiveAndroidEmoji {
    ...
    public static void init(Context context) {
        ...
        int[] codes = context.getResources().getIntArray(context.getResources()
                .getIdentifier("live_emoji_code", "array", context.getPackageName()));

        TypedArray array = context.getResources().obtainTypedArray(
                context.getResources().getIdentifier("live_emoji_res", "array", context.getPackageName()));

        TypedArray strArray = context.getResources().obtainTypedArray(
                context.getResources().getIdentifier("live_emoji_desc", "array", context.getPackageName()));
...
//        replaceEmojiMap.put(0x2601, "☁️");
    ...
    public static CharSequence ensure(String input) {
        ...
        if (useEmoji2()) {
            try {
                ...
                Log.i(TAG, "ensure input:" + e.toString());
                cs = input;
            }
        } else {
            cs = input;
    ...
    public static void ensure(Spannable spannable, float textSize) {
        ...
        if (useEmoji2()) {
            try {
                ...
                Log.i(TAG, "ensure spannable:" + e.toString());
                cs = spannable;
            }
        } else {
            cs = spannable;
    ...
    public static Drawable getEmojiDrawable(Context context, int index) {
        ...
        if (index >= 0 && index < sEmojiList.size()) {
            ...
            drawable = ContextCompat.getDrawable(context, emoji.resId);
        }
        return drawable;
    }
