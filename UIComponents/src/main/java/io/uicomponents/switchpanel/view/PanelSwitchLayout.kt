package io.uicomponents.switchpanel.view
...
import io.uicomponents.switchpanel.compat.KeyboardHeightCompat
import io.uicomponents.switchpanel.device.DeviceInfo
import io.uicomponents.switchpanel.device.DeviceRuntime
...
import io.uicomponents.switchpanel.log.LogFormatter
import io.uicomponents.switchpanel.log.LogTracker
import io.uicomponents.switchpanel.utils.DisplayUtil
import io.uicomponents.switchpanel.utils.DisplayUtil.getScreenHeightWithoutSystemUI
import io.uicomponents.switchpanel.utils.DisplayUtil.getScreenRealHeight
import io.uicomponents.switchpanel.utils.DisplayUtil.isPortrait
import io.uicomponents.switchpanel.utils.PanelUtil
import io.uicomponents.switchpanel.utils.PanelUtil.getKeyBoardHeight
import io.uicomponents.switchpanel.utils.isSystemInsetsAnimationSupport
import io.uicomponents.switchpanel.view.content.IContentContainer
...
class PanelSwitchLayout : LinearLayout, ViewAssertion {
    ...
    private var windowInsetsRootView: View? =
        null // 用于Android 11以上，通过OnApplyWindowInsetsListener获取键盘高度
    ...
    private var panelId = io.uicomponents.switchpanel.Constants.PANEL_NONE
    private var lastPanelId = io.uicomponents.switchpanel.Constants.PANEL_NONE
    private var lastPanelHeight = -1
    private var animationSpeed = 200 //standard
    private var enableAndroid11KeyboardFeature =
        true   // 是否启用 Android 11 键盘动画方案，目前发现 dialog，popupWindow等子窗口场景不支持键盘动画
    ...
    var softInputHeightCalculatorOnStart: ((animation: WindowInsetsAnimationCompat, bounds: WindowInsetsAnimationCompat.BoundsCompat) -> Int)? =
        null
    var softInputHeightCalculatorOnProgress: ((insets: WindowInsetsCompat, runningAnimations: MutableList<WindowInsetsAnimationCompat>) -> Int)? =
        null
    ...
    inner class CheckoutKbRunnable : Runnable {
        ...
        var delay: Long = 0L
        override fun run() {
            val result = checkoutPanel(io.uicomponents.switchpanel.Constants.PANEL_KEYBOARD)
            if (!result && panelId != io.uicomponents.switchpanel.Constants.PANEL_KEYBOARD && retry) {
    ...
    private fun initView(attrs: AttributeSet?, defStyleAttr: Int, defStyleRes: Int) {
        ...
        enableAndroid11KeyboardFeature =
            typedArray.getBoolean(R.styleable.PanelSwitchLayout_android11KeyboardFeature, true)
        typedArray.recycle()
    }
    ...
    private fun initListener() {
        ...
        contentContainer.getInputActionImpl()
            .setEditTextFocusChangeListener(OnFocusChangeListener { v, hasFocus ->
                notifyEditFocusChange(v, hasFocus)
                Log.d(
                    TAG,
                    "setEditTextFocusChangeListener: hasFocus = $hasFocus , panelId = $panelId"
                )
                if (isKeyboardState()) {
                    checkoutKeyboard()
                }
        ...
        for (i in 0 until array.size()) {
            ...
            keyView?.setOnClickListener(object : OnClickListener {
                override fun onClick(v: View) {
                    ...
                    if (currentTime - preClickTime <= io.uicomponents.switchpanel.Constants.PROTECT_KEY_CLICK_DURATION) {
                        LogTracker.log(
                            "$TAG#initListener",
    ...
    private fun getAndroidQNavHIfNavIsInvisible(runtime: DeviceRuntime, window: Window): Int {
        return if (!runtime.isNavigationBarShow && Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && DisplayUtil.hasSystemUIFlag(
                window,
            ...
            LogTracker.log(
                "$TAG#onGlobalLayout",
                " -> Android Q takes windowInset into calculation When nav is not shown and SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION flag is existed <-"
            )
            LogTracker.log("$TAG#onGlobalLayout", "stableInsetTop is : ${inset.stableInsetTop}")
            LogTracker.log(
                "$TAG#onGlobalLayout",
                "stableInsetBottom is : ${inset.stableInsetBottom}"
            )
            LogTracker.log(
                "$TAG#onGlobalLayout",
                "androidQCompatNavH is  ${inset.stableInsetBottom}"
            )
     ...
     */
    private fun getCurrentNavigationHeight(
        deviceRuntime: DeviceRuntime,
        deviceInfo: DeviceInfo
    ): Int {
        return if (deviceRuntime.isNavigationBarShow)
            deviceInfo.getCurrentNavigationBarHeightWhenVisible(
                deviceRuntime.isPortrait,
                deviceRuntime.isPad
            )
        else 0
    }
    ...
    internal fun bindWindow(window: Window, windowInsetsRootView: View?) {
        ...
        if (keyboardAnimationFeature) {
            ...
            deviceRuntime?.let {
                contentContainer.getInputActionImpl()
                    .updateFullScreenParams(it.isFullScreen, panelId, getCompatPanelHeight(panelId))
            ...
            object : WindowInsetsAnimationCompat.Callback(DISPATCH_MODE_CONTINUE_ON_SUBTREE) {
                ...
                override fun onStart(
                    ...
                    if (hasSoftInput && typeFlag != 0) {
                        ...
                        if (keyboardH == 0) {
                            LogTracker.log(
                                "onStart",
                                "键盘高度获取失败，请实现softInputHeightCalculatorOnStart兼容正确的键盘高度"
                            )
                            keyboardH =
                                softInputHeightCalculatorOnStart?.invoke(animation, bounds) ?: 0
                        }
                ...
                override fun onProgress(
                    ...
                    if (isPanelState(panelId)) {
                        ...
                        val fraction = imeAnimation?.fraction
                            ?: return insets                          // 动画进度 【0,1】
                        var softInputHeight =
                            insets.getInsets(WindowInsetsCompat.Type.ime()).bottom    // 键盘高度，这个高度包含了导航栏
                        if (hasSoftInput && fraction != 0f && softInputHeight == 0) { // 部分手机在键盘动画执行过程中无法获取到键盘高度
                            // 这里提供一个键盘计算的方法，业务方可以自行兼容
                            LogTracker.log(
                                "onProgress",
                                "键盘高度获取失败，请实现softInputHeightCalculatorOnProgress兼容正确的键盘高度，当前动画进度fraction = $fraction"
                            )
                            softInputHeight = softInputHeightCalculatorOnProgress?.invoke(
                                insets,
                                runningAnimations
                            ) ?: 0
                        }
                        val softInputTop =
                            window.decorView.bottom - softInputHeight            // 键盘top位于当前window的位置
                        ...
                        val location = DisplayUtil.getLocationOnWindow(this@PanelSwitchLayout)
                        val floatInitialBottom =
                            height + location[1]                           // PanelSwitchLayout控件位于当前window的Bottom
                        ...
                        if (hasSoftInput) { // 键盘显示时
                            if (softInputTop < floatInitialBottom) {
                                ...
                                if (panelContainer.translationY > offset) {
                                    ...
                                    contentContainer.translationContainer(
                                        contentScrollMeasurers,
                                        compatPanelHeight,
                                        offset
                                    )
    ...
    private fun keyboardChangedListener30Impl() {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(rootView) { view, insets ->
            ...
            LogTracker.log(
                "$TAG#WindowInsetsListener",
                "KeyBoardHeight : $keyboardH，isShow $imeVisible"
            )

            if (keyboardH != lastKeyboardHeight) {
                val contentHeight = getScreenHeightWithoutSystemUI(window)
                val androidQCompatNavH =
                    deviceRuntime?.run { getAndroidQNavHIfNavIsInvisible(this, window) } ?: 0
    ...
    private fun keyboardChangedListener(window: Window, it: DeviceRuntime) {
        globalLayoutListener = ViewTreeObserver.OnGlobalLayoutListener {
            ...
            logFormatter.addContent(
                "currentPanelSwitchLayoutVisible",
                "${this@PanelSwitchLayout.visibility == VISIBLE}"
            )
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                logFormatter.addContent(
                    "inset",
                    "left(${inset.stableInsetLeft}) top(${inset.stableInsetTop}) right(${inset.stableInsetRight}) bottom(${inset.stableInsetBottom})"
                )
            }
            logFormatter.addContent(
                "currentSystemInfo",
                "statusBarH : $curStatusHeight, navigationBarH : $cusNavigationHeight 全面屏手势虚拟栏H : $androidQCompatNavH"
            )
            ...
            minLimitCloseKeyboardHeight =
                if (info.navigationBarH > androidQCompatNavH) info.navigationBarH else androidQCompatNavH
            ...
            logFormatter.addContent(
                "currentKeyboardInfo",
                "keyboardH : $keyboardHeight, realKeyboardH : $realHeight, isShown : $isKeyboardShowing"
            )
     ...
     * 键盘高度变化时，状态更新
     */
    private fun handleKeyboardStateChanged(
        keyboardHeight: Int,
        realHeight: Int,
        contentHeight: Int
    ) {
        if (isKeyboardShowing) {
            if (keyboardHeight <= minLimitOpenKeyboardHeight) {
                ...
                if (isKeyboardState()) {
                    checkoutPanel(io.uicomponents.switchpanel.Constants.PANEL_NONE)
                }
                ...
                if (keyboardHeight != lastKeyboardHeight) {
                    LogTracker.log(
                        "$TAG#KeyboardStateChanged",
                        "try to set KeyBoardHeight : $realHeight，isShow $isKeyboardShowing"
                    )
            ...
            if (keyboardHeight > minLimitOpenKeyboardHeight) {
                ...
                if (!isKeyboardState()) {
                    checkoutPanel(io.uicomponents.switchpanel.Constants.PANEL_KEYBOARD, false)
                }
                ...
                lastContentHeight?.let { lastHeight ->
                    lastNavigationBarShow?.let { lastShow ->
                        if (lastHeight != contentHeight && lastShow != deviceRuntime?.isNavigationBarShow) {
                            ...
                            LogTracker.log(
                                "$TAG#KeyboardStateChanged",
                                "update layout by navigation visibility State change"
                            )
                        }
                    }
                }
    ...
    private fun notifyPanelChange(panelId: Int) {
        panelChangeListeners?.let {
            for (listener in it) {
                when (panelId) {
                    io.uicomponents.switchpanel.Constants.PANEL_NONE -> {
                        listener.onNone()
                    }

                    io.uicomponents.switchpanel.Constants.PANEL_KEYBOARD -> {
                        listener.onKeyboard()
                    }
    ...
    private fun getCompatPanelHeight(panelId: Int): Int {
        if (isPanelState(panelId)) {
            ...
            panelHeightMeasurer?.let {
                ...
                if (!PanelUtil.hasMeasuredKeyboard(context)
                    || !it.synchronizeKeyboardHeight()
                    || it.forceUseTargetPanelDefaultHeight()
                ) {
                    val result = it.getTargetPanelDefaultHeight()
                    LogTracker.log(
                        "$TAG#onLayout",
                        " getCompatPanelHeight by default panel  :$result"
                    )
                    return result
                }
            }
            ...
            panelHeightMeasurer?.let {
                //如果输入法还没有测量或者不同步输入法高度，则是有默认高度
                if (it.forceUseTargetPanelDefaultHeight()
                ) {
                    val result = it.getTargetPanelDefaultHeight()
                    LogTracker.log(
                        "$TAG#onLayout",
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        if (keyboardAnimationFeature) {
            ...
            val compatPanelHeight = getCompatPanelHeight(panelId)
            if (panelId != io.uicomponents.switchpanel.Constants.PANEL_NONE && compatPanelHeight != 0) {
        ...
        deviceRuntime?.let {
            ...
            if (it.isNavigationBarShow) {
                 ...
                 * 这样抽象逻辑变得更加简单。
                 */
                allHeight -= deviceInfo.getCurrentNavigationBarHeightWhenVisible(
                    it.isPortrait,
                    it.isPad
                )
            }

            val localLocation = DisplayUtil.getLocationOnScreen(this)
            ...
            val contentContainerHeight =
                getContentContainerHeight(allHeight, paddingTop, compatPanelHeight)
            ...
            if (io.uicomponents.switchpanel.Constants.DEBUG) {
                ...
                logFormatter.addContent(
                    "layoutInfo",
                    "onLayout(changed : $changed , l : $l  , t : $t , r : $r , b : $b)"
                )
                val state = when (panelId) {
                    io.uicomponents.switchpanel.Constants.PANEL_NONE -> "收起所有输入源"
                    io.uicomponents.switchpanel.Constants.PANEL_KEYBOARD -> "显示键盘输入"
                    else -> "显示面板输入"
                }
                ...
                logFormatter.addContent(
                    "screenH (static,include SystemUI)",
                    "${deviceInfo.screenH}"
                )
                logFormatter.addContent(
                    "screenH (static,exclude SystemUI)",
                    "${deviceInfo.screenWithoutNavigationH}"
                )
                logFormatter.addContent(
                    "screenH (dynamic,exclude SystemUI)",
                    "${deviceInfo.screenWithoutSystemUiH}"
                )
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                val changeBounds =
                    isBoundChange(l, contentContainerTop, r, panelContainerTop + compatPanelHeight)
            ...
            run {
                panelContainer.layout(
                    ...
                    panelContainerTop,
                    r,
                    panelContainerTop + compatPanelHeight
                )
                logFormatter.addContent(
                    "panelContainer Layout",
                    "($l,$panelContainerTop,$r,${panelContainerTop + compatPanelHeight})"
                )
                panelContainer.changeContainerHeight(compatPanelHeight)
            }
            this.lastPanelHeight = compatPanelHeight;
            contentContainer.getInputActionImpl()
                .updateFullScreenParams(it.isFullScreen, panelId, compatPanelHeight)
    ...
    private fun isKeyboardState(panelId: Int) = panelId == io.uicomponents.switchpanel.Constants.PANEL_KEYBOARD

    private fun isResetState(panelId: Int) = panelId == io.uicomponents.switchpanel.Constants.PANEL_NONE
    ...
    internal fun checkoutPanel(panelId: Int, checkoutKeyboard: Boolean = true): Boolean {
            ...
            io.uicomponents.switchpanel.Constants.PANEL_KEYBOARD -> {
                if (checkoutKeyboard) {
                    if (!contentContainer.getInputActionImpl().showKeyboard()) {
                        LogTracker.log(
                            "$TAG#checkoutPanel",
                            "system show keyboard fail, just ignore!"
                        )
                        doingCheckout = false
                        return false
                    }
                ...
                val size =
                    Pair(measuredWidth - paddingLeft - paddingRight, getCompatPanelHeight(panelId))
        ...
        LogTracker.log(
            "$TAG#checkoutPanel",
            "checkout success ! lastPanel's id : $lastPanelId , panel's id :$panelId"
        )
        requestLayout()
