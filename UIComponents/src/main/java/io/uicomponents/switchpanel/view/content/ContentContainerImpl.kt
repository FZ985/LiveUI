    ...
    init {
        ...
        mInputAction = object : IInputAction {
            ...
            override fun updateFullScreenParams(isFullScreen: Boolean, panelId: Int, panelHeight: Int) {
                ...
                if (isFullScreen) {
                    if (panelId == io.uicomponents.switchpanel.Constants.PANEL_KEYBOARD) {
                        retrieveFocusRight(requestFocus = true, resetSelection = true)
                    } else if (panelId != io.uicomponents.switchpanel.Constants.PANEL_NONE && !PanelUtil.isPanelHeightBelowKeyboardHeight(context, panelHeight)) {
