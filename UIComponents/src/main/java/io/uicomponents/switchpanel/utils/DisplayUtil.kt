...
object DisplayUtil {
    ...
    fun getNavigationBarHeight(context: Context, window: Window): Int {
        val deviceNavigationHeight = getInternalDimensionSize(context.resources, io.uicomponents.switchpanel.Constants.NAVIGATION_BAR_HEIGHT_RES_NAME)
    ...
    private fun getInternalDimensionSize(res: Resources, key: String): Int {
        var result = 0
        val resourceId = res.getIdentifier(key, io.uicomponents.switchpanel.Constants.DIMEN, io.uicomponents.switchpanel.Constants.ANDROID)
        if (resourceId > 0) {
    ...
    private fun hasNavBar(context: Context): Boolean {
        ...
        val resourceId = res.getIdentifier(
            io.uicomponents.switchpanel.Constants.SHOW_NAV_BAR_RES_NAME,
            "bool",
            "android"
        )
